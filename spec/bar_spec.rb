...
describe Bar do
  ...
  it "only returns a discount when it's happy hour" do
    ...
    #The stubbing idea. You take a :symbol and give it a return value. 
    #Thus, when you call [object].symbol it will return the value. 
    #The catch, you never had to set up the actual function!!!

    # To use a stubbed method, you have to define the method
    # in the class that is using it. But you ONLY have to 
    # define it, you don't need to make it do anything.

    expect(@bar).to receive(:happy_hour?).and_return(false) #stubbing happy_hour?
    expect(@bar.happy_discount).to eq 0
